...
_.base64ToBuffer = function(base64) {
...
_.randomProfilePic = function(gender) {
    if (!gender) return `https://rummyuser-avatar.s3.ap-south-1.amazonaws.com/avatars/Profile.png`;
    const male = [1, 3, 4, 5, 7, 9];
    const female = [2, 6, 8, 10];
    const eGender = gender === 'male' ? male : female;
    return `https://rummyuser-avatar.s3.ap-south-1.amazonaws.com/avatars/avatar-${_.randomFromArray(eGender)}.png`;
    // return `${process.env.S3_AVATAR_URL}/avatar-${_.randomFromArray(eGender)}.png`;
};
